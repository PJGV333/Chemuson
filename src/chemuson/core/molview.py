...
una API uniforme a los subsistemas químicos.
"""
...
from .model import bond_style_is_structural


class MolecularViewNotSupported(Exception):
    """Se lanza cuando un grafo no puede ser leído por ``MolView``."""
...
class MolView:
    ...
    def atoms(self) -> List[int]:
        ...
        Raises:
            MolecularViewNotSupported: Si el grafo no expone átomos accesibles.
        """
        graph = self.graph
        ...
        raise MolecularViewNotSupported("Cannot read atoms from graph")
    ...
    def element(self, atom_id: int) -> str:
        ...
        Raises:
            MolecularViewNotSupported: Si no se puede resolver el elemento.
        """
        atom = self._get_atom(atom_id)
        if atom is None:
            raise MolecularViewNotSupported("Cannot resolve atom element")
    ...
    def _atom_id(self, atom) -> int:
        ...
        Raises:
            MolecularViewNotSupported: Si no se puede extraer el ID.
        """
        ...
        if atom_id is not None:
            return int(atom_id)
        raise MolecularViewNotSupported("Atom object has no id")
