...
ELIMINATED_PERSISTENCE_EXCEPTION = _identity(
    "M01", "M09", "src/chemuson/chemio/persistence.py",
    "from chemuson.gui.canvas import ChemusonCanvas", True,
)
LEGACY_TEST_BASELINE_IDENTITY = ELIMINATED_PERSISTENCE_EXCEPTION
LEGACY_TEST_BASELINE_ROWS: tuple[ExceptionIdentity, ...] = (LEGACY_TEST_BASELINE_IDENTITY,)
...
def test_frozen_baseline_is_complete_and_exact() -> None:
    assert FROZEN_EXCEPTION_BASELINE_ROWS == ()
    assert FROZEN_EXCEPTION_BASELINE == frozenset()
    assert _duplicates(FROZEN_EXCEPTION_BASELINE_ROWS) == ()
    assert _counts_by_source(FROZEN_EXCEPTION_BASELINE_ROWS) == ()
    assert sum(not row.type_checking_only for row in FROZEN_EXCEPTION_BASELINE_ROWS) == 0
    assert sum(row.type_checking_only for row in FROZEN_EXCEPTION_BASELINE_ROWS) == 0
...
def test_real_catalog_matches_frozen_exception_baseline() -> None:
    ...
    assert audit.current_identities == () and audit.unexpected_identities == () and audit.removed_baseline_identities == () and audit.duplicate_identities == () and audit.baseline_duplicate_identities == () and audit.current_counts_by_module == () and audit.baseline_counts_by_module == () and audit.modules_with_growth == () and runtime == 0 and type_checking == 0, "\n".join(problems)
...
def test_replacing_persistence_exception_with_m01_m02_debt_is_rejected() -> None:
    ...
    assert len(audit.current_identities) == len(LEGACY_TEST_BASELINE_ROWS)
...
def test_new_identities_and_replacement_are_detected() -> None:
    ...
    assert replacement.removed_baseline_identities == (LEGACY_TEST_BASELINE_ROWS[0],)
...
@pytest.mark.parametrize(
    "duplicate",
    [
        LEGACY_TEST_BASELINE_ROWS[0],
        _changed(LEGACY_TEST_BASELINE_ROWS[0], "import_path", "from chemuson.gui.canvas\n import ChemusonCanvas"),
        _changed(LEGACY_TEST_BASELINE_ROWS[0], "file", r"src\chemuson\chemio\persistence.py"),
    ],
)
...
def test_multiple_growth_order_baseline_duplicates_and_metadata_duplicates() -> None:
    ...
    assert duplicate_metadata.duplicate_identities == ((LEGACY_TEST_BASELINE_ROWS[0], 2),)
    assert duplicate_baseline.baseline_duplicate_identities == ((LEGACY_TEST_BASELINE_ROWS[0], 2),)
