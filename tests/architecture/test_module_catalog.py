...
EXPECTED_IDS = {f"M{i:02d}" for i in range(23)}
...
class TestModuleIds:
    """Validate module identification: exactly 23 modules, M00-M22, unique, correct format."""

    def test_exactly_23_modules(self, modules):
        assert len(modules) == 23, f"Expected 23 modules, got {len(modules)}"

    def test_ids_are_m00_to_m22(self, modules):
