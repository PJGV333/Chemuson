...
def test_frozen_baseline_is_complete_and_exact() -> None:
    assert len(FROZEN_EXCEPTION_BASELINE_ROWS) == 10
    assert len(FROZEN_EXCEPTION_BASELINE) == 10
    assert _duplicates(FROZEN_EXCEPTION_BASELINE_ROWS) == ()
    assert _counts_by_source(FROZEN_EXCEPTION_BASELINE_ROWS) == (("M01", 6), ("M03", 2), ("M15", 2))
    assert sum(not row.type_checking_only for row in FROZEN_EXCEPTION_BASELINE_ROWS) == 8
    assert sum(row.type_checking_only for row in FROZEN_EXCEPTION_BASELINE_ROWS) == 2
...
def test_real_catalog_matches_frozen_exception_baseline() -> None:
    ...
    assert not audit.unexpected_identities and not audit.duplicate_identities and not audit.modules_with_growth and len(audit.current_identities) == 10 and audit.current_counts_by_module == (("M01", 6), ("M03", 2), ("M15", 2)) and runtime == 8 and type_checking == 2, "\n".join(problems)
...
def test_equal_empty_and_reduced_catalogs_are_no_growth() -> None:
    ...
    assert not empty.unexpected_identities and len(empty.removed_baseline_identities) == 10
...
def test_new_identities_and_replacement_are_detected() -> None:
    ...
    assert audit.modules_with_growth == (("M01", 7, 6), ("M02", 1, 0))
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("import_path", "\tfrom chemuson.clean2d.geometry\n import count_crossings,   cycle_basis, segments_intersect "),
        ("file", r"src\chemuson\chemio\depiction_candidates.py"),
        ("file", "./src/chemuson/chemio/depiction_candidates.py"),
        ("file", " src/chemuson/chemio/depiction_candidates.py "),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        _changed(FROZEN_EXCEPTION_BASELINE_ROWS[0], "import_path", "from chemuson.clean2d.geometry\n import count_crossings, cycle_basis, segments_intersect"),
        _changed(FROZEN_EXCEPTION_BASELINE_ROWS[0], "file", r"src\chemuson\chemio\depiction_candidates.py"),
    ],
)
def test_duplicates_after_normalization_are_detected(duplicate: ExceptionIdentity) -> None:
    ...
    assert audit.modules_with_growth == (("M01", 7, 6),)
