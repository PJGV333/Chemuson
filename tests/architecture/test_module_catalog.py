...
EXPECTED_IDS = {f"M{i:02d}" for i in range(22)}
...
class TestModuleIds:
    """Validate module identification: exactly 22 modules, M00-M21, unique, correct format."""

    def test_exactly_22_modules(self, modules):
        assert len(modules) == 22, f"Expected 22 modules, got {len(modules)}"

    def test_ids_are_m00_to_m21(self, modules):
